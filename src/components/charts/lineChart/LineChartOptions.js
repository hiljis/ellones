...
export const getChartCardlineChartOptions = (chartData, dataCategory, scale, resolution) => {
	return {
		// indexAxis: 'xAxis',
		responsive: true,
		maintainAspectRatio: false,
			...
			'y-axis-1': {
				...
				ticks: {
					callback: (value, i, t) => {
						...
						if (
							scale === 'logarithmic' &&
							(value === 0.1 || value === 1 || value === 10 || value === 100)
						) {
							return value;
						} else if (
							scale === 'logarithmic' &&
							...
							return `${value / 1000} K`;
						} else if (scale === 'logarithmic' && value === 1000000) {
							return '1 M';
						} else if (scale === 'logarithmic') return;
